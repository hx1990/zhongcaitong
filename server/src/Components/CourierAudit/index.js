...
class CourierAudit extends Component {
     constructor(){
        super()
        this.state={
          data :[],
    ...
    handleOk(){
      let that=this
      let postmsg={}
      log('aaa',that.state.partner.deliverId)
      if(this.state.unPassButton){
          postmsg={
            deliverId:that.state.partner.deliverId,
            status:5,
    ...
    render(){ 
        return (<div className="express">
           <div className="DetailList">送票员审核</div>
          ...
          <Modal title="个人详情" visible={this.state.visible} okText="确认" cancelText="取消" onOk={this.handleOk.bind(this)} onCancel={this.handleCancel.bind(this)}>
              <div className='parter-info'>
                ...
                <div className='item'>身份证正面：
                   <img className={this.state.pic?'upload-pic':''} data-id='0' onClick={this.clickPic.bind(this)} alt='' src={this.state.partner.idCardUp}/>
                </div>
